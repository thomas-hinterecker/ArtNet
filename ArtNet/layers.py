...
class Trainable(Layer):
    ...
    def _InitLayer(self, activation="Linear", weight_initializer='GlorotUniform', bias_initializer='Zeros',
...
class Conv2D(Trainable):
    ...
    def __init__(self, filters, kernel_size, strides=(1, 1), padding='valid', data_format=None, activation="Linear",
        ...
        # init layer stuff
        self._InitLayer(activation=activation, weight_initializer=weight_initializer, bias_initializer=bias_initializer,
...
class MaxPooling2D(Conv2D):
    ...
    def Backward(self, model, layer, dOut0):
        ...
        dOut = dOut.transpose(0, -1, 1, 2).reshape(self.input_data.shape[-1], self.input_data.shape[0],
                                                   self.input_data.shape[1], self.input_data.shape[2]).transpose(1, 3,
                                                                                                                 2, 0)
